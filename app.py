...
# Brands the engine will try to "lock" (always prefer same-brand if available).
LOCKABLE_BRANDS = {
...
def _sweet_spot_min_ratio(trigger_price: float) -> float:
    if trigger_price >= 700: return 0.30
    if trigger_price >= 400: return 0.40
    return 0.50
...
PENALTY_OVER_CAP_SOFT     =  -200_000
PENALTY_OVER_CAP_HARD     =  -400_000
...
WEARABLE_MARKETING_COPY: dict[str, str] = {
    "ΛΟΥΡΑΚΙΑ WEARABLES":          "Ανανέωσε το στυλ σου — από sport silicone έως premium leather.",
    "ΠΡΟΣΤΑΣΙΑ ΟΘΟΝΗΣ WEARABLES":  "Διατήρησε την οθόνη σου αλώβητη από γρατζουνιές και πτώσεις.",
    "Bluetooth":                   "Απόλαυσε ασύρματο ήχο στις προπονήσεις και στην καθημερινή σου.",
    "BODY SCALES":                 "Παρακολούθησε πλήρη σύσταση σώματος — πέρα από απλό βάρος.",
...
SPEC_SIGNATURE_COLUMNS: dict[str, list[str]] = {
    "ΠΡΟΣΤΑΣΙΑ ΟΘΟΝΗΣ WEARABLES": ["Τύπος3"],
    "ΛΟΥΡΑΚΙΑ WEARABLES":         ["Υλικό", "Χρώμα"],     # silicone vs nylon vs leather
    "Bluetooth":                  ["Τύπος ακουστικών", "Προτεινόμενη χρήση"],
    "BODY SCALES":                ["Συνδεσιμότητα", "Δυνατότητες"],
    "WALL CHARGERS":              ["Ισχύς (Watt)", "Τύπος3"],
    "ΦΟΡΤΙΣΤΕΣ WEARABLES":        ["Τύπος3"],
    "OVERHEAD":                   ["Τύπος ακουστικών"],   # On Ear vs Over Ear vs In Ear
    "ΗΧΕΙΑ ΦΟΡΗΤΟΥ ΗΧΟΥ":         ["Ισχύς (Watt).1", "Συνδεσιμότητα"],
    "ΒΑΣΕΙΣ":                     ["Τύπος3"],             # Αυτοκινήτου vs Ποδηλάτου vs Επιτραπέζια
    "CAR CHARGERS":               ["Ισχύς (Watt)", "Τύπος3"],
}
 
 
def _signature(row: pd.Series, hierarchy: str) -> tuple:
    """Build a tuple signature for a row using the hierarchy's spec columns."""
    cols = SPEC_SIGNATURE_COLUMNS.get(hierarchy, [])
    return tuple(
        str(row.get(c, "")).strip().upper() if pd.notna(row.get(c, "")) else ""
        for c in cols
    )
...
# ═════════════════════════════════════════════════════════════
# 🟢 TRADITIONAL VACUUMS CONFIGURATION (Ηλεκτρικές Σκούπες — Μικρές Συσκευές)
# ═════════════════════════════════════════════════════════════
# Trigger detection: products in Floor sheet with Hierarchy = "Ηλεκτρικές Σκούπες".
# Same rules as Robot Vacuums:
#   • STRICT brand+model match (non-negotiable) for Σακούλες and Εξαρτήματα
#   • UNIVERSAL αρωματικά / πέρλες as a separate slot
#   • Companions: sales + brand boost + price-tier proximity
#   • PET CARE: sales-only, gated on Κατάλληλη για κατοικίδια
#   • Σκούπες Stick and Σκούπες ρομπότ capped at 1 slot each (premium / overlap concerns)

TRAD_VAC_TRIGGER_HIERARCHIES = {
...
# (priority_rank, role_label, hierarchies, logic_key, max_in_round_1, max_total)
# Priority follows the user's stated order. STRICT_ACCESSORY = brand AND model
# match required, no universal fallback. UNIVERSAL_AROMA = pull αρωματικά/πέρλες
# from Εξαρτήματα. COMPANION/PET_CARE shared with Robot Vacuums engine.
TRAD_VAC_PRIORITY = [
    (1, 'Σακούλες',         ['Σακούλες για σκούπες'],     'STRICT_ACCESSORY', 2, None),
    (2, 'Αρωματικά',        ['Εξαρτήματα για σκούπες'],   'UNIVERSAL_AROMA',  1, None),
    (3, 'Σκουπάκι',         ['Ηλεκτρικά Σκουπάκια'],      'COMPANION',        1, None),
    (4, 'Σκούπα Stick',     ['Σκούπες Stick'],            'COMPANION',        1, 1),
    (5, 'Εξαρτήματα',       ['Εξαρτήματα για σκούπες'],   'STRICT_ACCESSORY', 2, None),
    (6, 'Σκούπα Ρομπότ',    ['Σκούπες ρομπότ'],           'COMPANION',        1, 1),
    (7, 'Ατμοκαθαριστής',   ['Ατμοκαθαριστές'],           'COMPANION',        1, None),
    (8, 'Pet Care',         ['PET CARE'],                 'PET_CARE',         1, None),
]

TRAD_VAC_SLOT_TARGET = 10

TRAD_VAC_MARKETING_COPY = {
    "Σακούλες":       "Σωστή σακούλα = μέγιστη απόδοση & καθαρός αέρας.",
    "Αρωματικά":      "Φρεσκάδα σε κάθε σκούπισμα.",
    "Σκουπάκι":       "Άμεσος καθαρισμός σε δύσκολα σημεία.",
    "Σκούπα Stick":   "Ευελιξία χωρίς καλώδιο.",
    "Εξαρτήματα":     "Συμβατό εξάρτημα για τη σκούπα σου.",
    "Σκούπα Ρομπότ":  "Καθαρίζει μόνη της όσο λείπεις.",
    "Ατμοκαθαριστής": "Απολύμανση με ατμό — χωρίς χημικά.",
    "Pet Care":       "Φροντίδα για το κατοικίδιό σου.",
}
...
# ═════════════════════════════════════════════════════════════
# 🟢 STRAIGHTENERS CONFIGURATION (Ισιωτικά Μαλλιών — Personal Care)
# ═════════════════════════════════════════════════════════════
# Trigger detection: products in SDA sheet with Hierarchy = "STRAIGHTENERS"
# (Level 1 = Personal Care, Level 2 = Women's Care).
#
# Recommendation depth — HYBRID (sales × brand-ecosystem × price-tier × color):
#   • Straighteners have 107 SKUs but limited structured specs beyond brand,
#     price, color, and device-type. "Ειδικά χαρακτηριστικά" and "Δυνατότητες"
#     are unstructured free text (lots of unique values) — unusable for
#     cross-matching. So full-spec cross-matching would add noise, not signal.
#   • Pure sales-only would miss the strongest cross-sell signal in this
#     category: BRAND ECOSYSTEM. 13/14 straightener brands also make hair
#     dryers; 10/14 also make curlers. A €459 DYSON Airstrait buyer expects
#     to see a DYSON Supersonic, not a €15 REMINGTON dryer.
#   • Three specs deployed alongside sales:
#       1) Κατασκευαστής (brand) — direct match, parsed from Title for the
#          male-grooming hierarchies where the column is empty (not used by
#          straighteners but kept reusable for future Personal Care engines)
#       2) LIST PRICE / price tier — proximity (Entry/Mainstream/Premium/Pro)
#       3) Χρώμα (color group) — soft boost when colors match aesthetic
#          (Ροζ↔Ροζ, Μαύρο↔Μαύρο). Color matters more here than in any other
#          cluster except fridges, because Personal Care is a fashion-adjacent
#          purchase — women coordinate their hair-styling tools by color.
#
# Slot composition (10 slots, 8 hierarchies):
#   1. HAIR DRYERS         — top sales (1408), natural pair, max_total=2
#   2. CURLERS & BRUSHES   — hair-styling sibling, strong overlap, max_total=2
#   3. MULTISTYLERS        — premium alt-style (Dyson Airwrap class)
#   4. ACCESSORIES         — heat protectants, oils, brushes (Personal Care)
#   5. EPILATORS           — women's-care companion
#   6. ELECTRIC TOOTHBRUSHES — universal personal-care wellness
#   7. BODY SCALES         — wellness
#   8. MASSAGE DEVICES     — wellness
#
# Round 1 = 1 per pool × 8 pools = 8 slots; round 2 overflows from HAIR DRYERS
# and CURLERS (the only pools with max_total=2) to fill slots 9-10. This keeps
# slots 1-2 as the hero hair-styling pair and slots 9-10 as natural overflow
# rather than wedging more wellness items.
#
# We deliberately EXCLUDE the men's-grooming hierarchies (TRIMMERS, SHAVING
# MACHINES, GROOMING SET, ΚΟΥΡΕΥΤΙΚΕΣ ΜΗΧΑΝΕΣ) for the straightener trigger.
# Zero brand overlap (besides REMINGTON), and a women's hair-styling carousel
# showing nose-hair trimmers reads as a categorization error. Those four
# hierarchies will get their own dedicated engine when we add SHAVING MACHINES
# as a trigger in a later iteration.

STRAIGHTENER_TRIGGER_HIERARCHIES = {
    "STRAIGHTENERS", "Straighteners", "straighteners",
    "ΙΣΙΩΤΙΚΑ ΜΑΛΛΙΩΝ", "Ισιωτικά Μαλλιών", "Ισιωτικό Μαλλιών",
}

# Test SKUs — 6 representative straighteners spanning the full spectrum.
# Set covers: premium DYSON multistyler hero (#1 seller), DYSON Corrale
# pure-straightener premium, mid-range PHILIPS (#3 seller, mainstream price),
# steam-tech BELLISSIMA (steam straightener subtype), entry-level IZZY
# (budget mainstream), entry-level REMINGTON (budget classic with keratin).
# Empty set = show all 107.
STRAIGHTENER_TEST_SKUS = {
    "2098479",  # DYSON AIRSTRAIT HT01 €459 Ροζ — #1 seller, premium hero
    "1898709",  # DYSON CORRALE Special Edition €499 Blue Blush — premium classic
    "1658773",  # PHILIPS BHS378/00 €45 Μαύρο — mainstream best-seller (18.5k)
    "1984742",  # BELLISSIMA STEAM CERAMIC & KERATIN €70 — steam subtype
    "480466",   # IZZY HS-568 €61 Μαύρο — entry-level budget
    "1339080",  # REMINGTON KERATIN PROTECT S8540 €50 Χρυσό — keratin classic
}

# (priority_rank, role_label, hierarchies, logic_key, max_in_round_1, max_total)
# Looping engine: cycle priority order taking 1 per pool per round until 10
# slots filled. Pools with max_total=2 (HAIR DRYERS, CURLERS) overflow into
# round 2 to fill the carousel; the other 6 are capped at 1.
STRAIGHTENER_PRIORITY = [
    (1, 'Πιστολάκι Μαλλιών',
        ['HAIR DRYERS'],
        'STR_HAIRSTYLING', 1, 2),
...
STRAIGHTENER_SLOT_TARGET = 10

STRAIGHTENER_MARKETING_COPY = {
...
# Brand-ecosystem hints — these are the brands we know span multiple Personal
# Care hierarchies in the data, used purely for diagnostics. The brand boost
# is awarded dynamically by comparing the trigger brand against pool brands.
STRAIGHTENER_KNOWN_ECOSYSTEM_BRANDS = {
    'DYSON', 'PHILIPS', 'REMINGTON', 'BELLISSIMA', 'ROWENTA', 'VALERA',
    'BRAUN', 'BABYLISS', 'REVLON', 'IZZY', 'ROHNSON', 'PROFICARE',
    'TESLA', 'IQ', 'REVAMP', 'BEURER', 'ORAL-B',
}

# Price tiers for proximity scoring across Personal Care. Straighteners span
# €19-€559 — much wider than kitchen appliances — so we use 4 tiers. DYSON
# (€459+) sits in Pro; entry sub-€40 is the budget shelf.
def _str_price_tier(price: float) -> int:
...
# Color grouping for the Personal Care aesthetic boost. We bucket the noisy
# Greek color labels into 5 broad families so "Ροζ" matches "Ροζ Χρυσό", and
# "Μαύρο;Χρυσό" matches both Μαύρο and Χρυσό. Match logic: if ANY token in
# the trigger color overlaps with ANY token in the candidate color, it's a
# soft match (smaller boost than exact equality).
def _str_color_group(color: str) -> set:
    """Returns the set of color-family tokens for a (possibly multi-color) cell."""
    if not color or pd.isna(color):
        return set()
    c = str(color).strip()
    if not c or c == '—':
        return set()
    # Split on semicolon or slash (some rows use 'Μαύρο;Χρυσό' or 'Μαύρο/Χρυσό')
    parts = re.split(r'[;/]', c)
    groups = set()
    for p in parts:
        p_low = p.strip().lower()
        if not p_low:
            continue
        if 'μαύρ' in p_low or 'black' in p_low:
            groups.add('BLACK')
        elif 'ροζ' in p_low or 'pink' in p_low or 'rose' in p_low or 'champagne' in p_low:
            groups.add('PINK')
        elif 'λευκ' in p_low or 'white' in p_low:
            groups.add('WHITE')
        elif 'χρυσ' in p_low or 'gold' in p_low or 'bronze' in p_low or 'μπρονζ' in p_low:
            groups.add('GOLD')
        elif 'ασημ' in p_low or 'silver' in p_low or 'inox' in p_low or 'γκρι' in p_low or 'gray' in p_low or 'grey' in p_low or 'ανθρακ' in p_low:
            groups.add('SILVER')
        elif 'μπλε' in p_low or 'blue' in p_low or 'γαλάζ' in p_low or 'γαλαζ' in p_low:
            groups.add('BLUE')
        elif 'κόκκιν' in p_low or 'κοκκιν' in p_low or 'red' in p_low:
            groups.add('RED')
        elif 'μωβ' in p_low or 'purple' in p_low or 'λιλά' in p_low or 'λιλα' in p_low or 'violet' in p_low:
            groups.add('PURPLE')
        elif 'πράσιν' in p_low or 'πρασιν' in p_low or 'green' in p_low:
            groups.add('GREEN')
        elif 'καφέ' in p_low or 'καφε' in p_low or 'brown' in p_low or 'μπεζ' in p_low or 'beige' in p_low:
            groups.add('BROWN')
    return groups

# Detect whether the trigger is a multi-function straightener (also a multistyler)
# via the Τύπος συσκευής column. If yes, the MULTISTYLERS slot becomes more of
# an upsell ("you already have this functionality, here's the pro tier") so we
# bias the score even harder toward premium-tier multistylers.
def _str_is_multistyler_trigger(trigger_row) -> bool:
    """True if Τύπος συσκευής indicates multi-function (Πρέσα + Ισιωτικό combo)."""
    t = str(trigger_row.get('Τύπος συσκευής', '') or '').lower()
    return ('πρέσα' in t and 'ισιωτικ' in t) or 'πολυ' in t

# Title-brand parser — Personal Care hierarchies in the SDA sheet have the
# Κατασκευαστής column populated for women's-care items (Hair Dryers, Curlers,
# Straighteners, Multistylers, Epilators) but EMPTY for men's-grooming
# hierarchies (Trimmers, Shaving Machines, Grooming Set, Κουρευτικές Μηχανές).
# Straighteners don't cross-sell into those, so this helper is not invoked by
# the straighteners engine — but it's defined here for the future personal-care
# engines (e.g. Shaving Machines) that will need it.
def _str_parse_brand_from_title(title: str) -> str:
    """Extract brand as the first ALL-CAPS or known-brand token from title."""
    if not title or pd.isna(title):
        return ''
    t = str(title).strip()
    # Known brands seen in Personal Care SDA data — checked first as a fast path
    KNOWN = ['REMINGTON', 'PHILIPS', 'BRAUN', 'PANASONIC', 'BABYLISS', 'WAHL',
            'ROWENTA', 'BELLISSIMA', 'DYSON', 'IZZY', 'TESLA', 'VALERA',
            'REVLON', 'REVAMP', 'PROFICARE', 'PROFI CARE', 'ROHNSON',
            'BEURER', 'ORAL-B', 'ORAL B', 'SENCOR', 'AENO', 'LAIFEN',
    ...
    for brand in KNOWN:
        if t_upper.startswith(brand + ' ') or f' {brand} ' in t_upper or t_upper.startswith(brand + '-'):
            return brand.strip()
    # Fallback: first whitespace-separated token if it's all caps
    first = t.split()[0] if t.split() else ''
    if first.isupper() and len(first) >= 2 and first.isalpha():
        return first
    return ''

# Scoring constants — calibrated so brand-ecosystem outranks pure sales,
# price-tier proximity acts as a sieve, and color is the softest tiebreaker.
# Mirrors the AF_S_* convention.
STR_S_AVAILABILITY       = 100_000   # In-stock boost (Άμεσα Διαθέσιμο)
STR_S_BRAND_MATCH        = 500_000   # Same Κατασκευαστής as trigger (ecosystem hero)
STR_S_PRICE_SAME_TIER    = 250_000   # Companion in same price tier
STR_S_PRICE_ONE_OFF      =  80_000   # Companion ±1 price tier
STR_S_PRICE_TWO_OFF      = -150_000  # Companion ≥2 tiers away — penalize (€459 trigger ≠ €15 dryer)
STR_S_COLOR_EXACT        =  60_000   # Color-group exact match (Ροζ↔Ροζ)
...
def _brand_incompat_filter(pool, trigger_brand, known_brands=None,
                            also_check_title=False):
    """Drop accessories whose compat field (and optionally title) mentions
    a competitor brand but NOT the trigger brand.
 
    `also_check_title=True` catches "Tune Stylus για Apple iPad" cases
    where the brand is in the title, not the structured compat field."""
    if pool.empty or not trigger_brand:
        return pool
    ...
    if also_check_title:
        title = _series(pool, 'Title').str.upper()
        text = text + ' ' + title
...
def _apply_color_boost(pool, color_toks, with_transparent=True):
    """Add color-match boost to pool's Final_Score column. No filter — boost only."""
    if not color_toks:
        return pool
...
def _size_band_matches(band, tablet_size_inches):
    ...
    if m:
        # Single-size bag (e.g. '15.6"' or '14"'). Tablet must fit IN it,
...
def _apply_budget_filter(pool, role, ttier):
    """Hard-filter to [min, max] for role × tier, with safety net."""
    if pool.empty:
        return pool
...
def _universal_tablet_bag_pool(c, used_materials, tb, tsize, color_toks=None):
    ...
    # Size filter — ASYMMETRIC. The tablet has to fit INSIDE the bag, so:
    #   bag_size >= tablet_size  (no slack downward — a 10" bag DOES NOT fit
    #                              an 11" tablet, even though they're "close")
    #   bag_size <= tablet_size + 3.0  (generous upper bound — universals run
    #                                    large; a 14" sleeve for an 11" tablet
    #                                    is sloppy but won't damage it)
    # Bags with no declared size (genuine size-agnostic universals) are kept.
    if tsize > 0:
...
def parse_euro_price(raw) -> float:
...
def _price_affinity_score(item_price: float, cap: float, trigger_price: float) -> int:
    ...
    sweet_lo = _sweet_spot_min_ratio(trigger_price)
    if ratio <= sweet_lo: return SCORE_BELOW_SWEET
    if ratio <= 1.00:     return SCORE_IN_BUDGET_SWEET
    if ratio <= 1.50:     return PENALTY_OVER_CAP_SOFT
    if ratio <= 2.50:     return PENALTY_OVER_CAP_HARD
    return PENALTY_WAY_OVER_CAP
 
 
def _build_hierarchy_pool(
    ...
    mask = (
        ...
        .fillna("")
        .astype(str)
        .str.contains(hier_pattern, case=False, na=False, regex=True)
    )
    pool = candidates[mask].copy()
    return pool[~pool["Material"].isin(used_materials)]
...
def _score_pool(
    ...
    *,
    ...
    notes.append(f"Cap=€{cap:.0f} (dynamic from trigger €{trigger_price:.0f})")
    ...
    if logic_key == "AUDIO_USE_CASE":
        # In a unified flow, both TWS earbuds and sport audio are valid for slot 3.
        # Boost both patterns; tie-break later via brand/price/popularity.
        audio_mask = pool["Title"].str.contains(
            r"Buds|AirPods|FreeBuds|TWS|True\s*Wireless|Galaxy\s*Buds|"
            r"Sport|Run|Bone|Shokz|Neckband|IPX[567]",
            case=False, na=False, regex=True,
        )
        pool.loc[audio_mask, "_score"] += SCORE_USE_CASE_AFFINITY
        notes.append(f"Audio use-case boost: {int(audio_mask.sum())} items")
    elif logic_key == "MOUNT_USE_CASE":
        mount_mask = pool["Title"].str.contains(
            r"Dock|Stand|Station|Βάση|Bike|Ποδήλατο|Handlebar|Cycling|"
            r"Charging\s*Stand|Nightstand|GPS\s*Mount",
            case=False, na=False, regex=True,
        )
    ...
    if used_signatures and SPEC_SIGNATURE_COLUMNS.get(hierarchy):
        sig_series = pool.apply(lambda r: _signature(r, hierarchy), axis=1)
        # Penalise rows whose signature exactly matches one already used
        dup_mask = sig_series.isin(used_signatures)
        pool.loc[dup_mask, "_score"] += PENALTY_SPEC_DUPLICATE
        notes.append(f"Spec-variety: {int(dup_mask.sum())} dup-signature items penalised")
 
    pool = pool.sort_values(["_score", "Sales_Tiebreaker"], ascending=[False, False])
...
L1_CATEGORIES = [
...

    {
        "key": "Personal Care",
        "label": "Φροντίδα\nΣώματος",
        "icon_svg": "%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23ff5e00' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpath d='M12 2c-2 2-3 4-3 6a3 3 0 0 0 6 0c0-2-1-4-3-6z'/%3E%3Cpath d='M9 12v7'/%3E%3Cpath d='M15 12v7'/%3E%3Cpath d='M7 19h10'/%3E%3C/svg%3E",
    },
...
# ───── Product selector + trigger setup based on active_cluster ─────
    ...
    if active_cluster == "Smartphones":
        ...
        if watches.empty:
            st.sidebar.warning("Δεν βρέθηκαν Smartwatches.")
        else:
            st.sidebar.markdown('<p class="sidebar-section">Επιλέξτε Smartwatch</p>', unsafe_allow_html=True)
            sel = st.sidebar.selectbox("", watches['Title'].unique(), label_visibility="collapsed", key="watch_sel")
            trigger = watches[watches['Title']==sel].iloc[0] if sel else None
        ...
        if df_air.empty: 
            ...
            target_skus = {
                "2123107", "2037695", "2028089", "1938304", "1918665", "2128974"
            }
        ...
        # Trigger pool: traditional electric vacuums from the Floor sheet.
        # If TRAD_VAC_TEST_SKUS is non-empty, restrict the dropdown to those
        # specific SKUs (placeholder = top 5 by sales). Empty set = show all.
        if df_floor is None or df_floor.empty:
            st.sidebar.warning("Sheet 'Floor' is empty or missing.")
        else:
            hier_upper = df_floor['Hierarchy'].fillna('').astype(str).str.upper().str.strip()
            trigger_hiers_upper = {h.upper().strip() for h in TRAD_VAC_TRIGGER_HIERARCHIES}
            trad_vacs = df_floor[hier_upper.isin(trigger_hiers_upper)].copy()

            # 🧪 Optional test-list filter (leave TRAD_VAC_TEST_SKUS empty to show all)
            if TRAD_VAC_TEST_SKUS:
                mat_clean = trad_vacs['Material'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
                trad_vacs = trad_vacs[mat_clean.isin(TRAD_VAC_TEST_SKUS)]

            if trad_vacs.empty:
                st.sidebar.warning("Δεν βρέθηκαν Ηλεκτρικές Σκούπες στο sheet Floor.")
            else:
        ...
        # Level 1 = Personal Care, Level 2 = Women's Care.
        # If STRAIGHTENER_TEST_SKUS is non-empty, restrict to those 6 demo SKUs
        # spanning premium DYSON / mid PHILIPS / steam BELLISSIMA / budget IZZY+REMINGTON.
        if df_sda is None or df_sda.empty:
            st.sidebar.warning("Sheet 'SDA' is empty or missing.")
        else:
            hier_upper = df_sda['Hierarchy'].fillna('').astype(str).str.upper().str.strip()
            trigger_hiers_upper = {h.upper().strip() for h in STRAIGHTENER_TRIGGER_HIERARCHIES}
            straighteners = df_sda[hier_upper.isin(trigger_hiers_upper)].copy()

            # 🧪 Optional test-list filter (leave STRAIGHTENER_TEST_SKUS empty to show all 107)
            if STRAIGHTENER_TEST_SKUS:
                mat_clean = straighteners['Material'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
                straighteners = straighteners[mat_clean.isin(STRAIGHTENER_TEST_SKUS)]

            if straighteners.empty:
                st.sidebar.warning("Δεν βρέθηκαν Ισιωτικά Μαλλιών στο sheet SDA.")
            else:
...
def run_tablets_engine(trigger, df_products, df_history):
    ...
    if tsize <= 0:
        tsize = _extract_size_from_title(tt)   # title fallback
    ...
    for slot_num, role, hierarchies, logic_key in slots:
        ...
        # Tier 3: NB BAG with size match
            ...
            if in_target.any():
                folio_pool = pool[in_target].copy()
        ...
        # ───── TABLET KEYBOARD fit ─────
            ...
            pool = _apply_color_boost(pool, color_toks, with_transparent=False)
 
        # ───── WIRELESS KEYBOARD fit (brand-first + Bluetooth req) ─────
        elif base == 'KEYBOARD_WIRELESS_FIT':
            pool = _brand_first_filter(pool, tb)   # NEW: Samsung kbd if any exist
        ...
        # ───── WALL CHARGER fit ─────
            ...
            if wall.any():
                pool = pool[wall]
            # Brand-first BEFORE any other scoring/filtering
            pool = _brand_first_filter(pool, tb)
        ...
        # ───── APPLE CHARGER fit (iPad) ─────
            ...
            # Brand-first BEFORE budget — Apple charger wins even at €25
            pool = _brand_first_filter(pool, tb)
        ...
        # ───── APPLE CABLE fit ─────
            ...
            if is_cable.any():
                pool = pool[is_cable]
            pool = _brand_first_filter(pool, tb)   # brand-first BEFORE budget
            in_apple = pool['Hierarchy'].isin(
                ['APPLE ORIGINAL IPHONE CABLE-ADAPTORS',
                 'APPLE ORIGINAL IPHONE CABLE-ADA'])
            pool.loc[in_apple, 'Final_Score'] += S_HIERARCHY_TARGET
        ...
        # ───── STORAGE fit (USB-C hard filter for Type-C triggers) ─────
            ...
            if 'android' in tos:
                pool.loc[pool['Hierarchy'] == 'MICRO SD',
                         'Final_Score'] += S_PORT_MATCH
            sigma = _series(pool, 'Σύνδεση').str.lower()
            if 'type-c' in tp_l or 'usb-c' in tp_l:
                is_typec_flash = sigma.str.contains('usb-c', na=False)
                if is_typec_flash.any():
                    # Hard-filter — no USB-A flash drives on a Type-C iPad
                    flash_pool = pool[pool['Hierarchy'] != 'MICRO SD']
                    typec_in_flash = flash_pool['Σύνδεση'].fillna('').str.lower().str.contains('usb-c', na=False) if 'Σύνδεση' in flash_pool.columns else None
                    keep = (pool['Hierarchy'] != 'USB FLASH DISK') | is_typec_flash
                    pool = pool[keep]
            pool.loc[_port_mask_flash(pool, tport),
                     'Final_Score'] += S_PORT_MATCH
            pool.loc[pool['Hierarchy'].eq('ΚΑΛΩΔΙΑ-ADAPTORS'),
                     'Final_Score'] += S_OTG_BONUS
            # Premium boost: bigger capacity sticks (proxy = price)
            if is_premium:
                pool.loc[pool['_p'] >= 30, 'Final_Score'] += S_PREMIUM_VARIANT // 2
        ...
        # ───── BRAND match (Apple Watch on iPad, Smartwatch on standard) ─────
            ...
            pool = _brand_first_filter(pool, tb)
            # Color boost — match watch color to tablet color
            pool = _apply_color_boost(pool, color_toks, with_transparent=False)
            # Premium tier → boost expensive watches (Series/Ultra)
            if is_premium:
                pool.loc[pool['_p'] >= 400, 'Final_Score'] += S_PREMIUM_VARIANT
 
        # ───── SCREEN PROTECTOR fit (skip-on-failure) ─────
        elif base == 'SCREEN_PROTECTOR_FIT':
            pool = _brand_incompat_filter(pool, tb, also_check_title=True)
            if tmod:
                mm = _compat_mask(pool, tmod)
                if mm.any():
                    pool = pool[mm]
                else:
                    # Skip the slot entirely — no fallback (drop to user)
                    slot_notes[slot_num] = notes + ['NO_MODEL_COMPAT_SKIP']
                    continue   # NOT added to failed_slots → no fallback
            else:
                slot_notes[slot_num] = notes + ['NO_MODEL_TO_MATCH_SKIP']
                continue
 
        # ───── STYLUS fit (brand-first + title-aware incompat) ─────
        elif base == 'STYLUS_FIT':
            # Drop styluses that mention competitor brands in compat OR title
            pool = _brand_incompat_filter(pool, tb, also_check_title=True)
            # If trigger is a major brand and any same-brand stylus exists, hard-filter
            if tb in TABLET_PREMIUM_BRANDS:
                pool = _brand_first_filter(pool, tb)
            if tmod:
                mm = _compat_mask(pool, tmod)
                pool.loc[mm, 'Final_Score'] += S_MODEL_MATCH
        ...
        if base in ('APPLE_TARGET', 'APPLE_TARGET_STRICT', 'APPLE_FOLIO_STRICT'):
...
def run_wearables_engine(
    ...
    for slot_num, role, hierarchies, logic_key, budget_key, ecosystem_mode, title_filter, smartphone_style in SLOTS:
        ...
        if chosen is None:
            ...
            diag.append((
                f"Slot {slot_num} {empty_marker}",
                role,
                f"Left empty — will compensate via slot 11+",
            ))
        ...
        cap_used = get_dynamic_cap(tprice, budget_key)
        score_breakdown[slot_num] = {
            "slot_role":   role,
            "material":    chosen["Material"],
            "title":       str(chosen.get("Title", ""))[:60],
            "brand":       chosen_brand,
            "final_score": round(chosen["_score"]),
            "price":       round(chosen["_p"], 2),
            "cap":         round(cap_used, 2),
            "price_ratio": round(chosen["_p"] / cap_used, 2) if cap_used else None,
            "fallback":    False,
            "locked":      locked,
            "logic":       logic_key,
            "signature":   _signature(chosen, hierarchy),
        }
        lock_flag = "🔒" if locked else ""
        diag.append((
            f"Slot {slot_num} ✅{lock_flag}",
            f"{role}  →  {str(chosen.get('Title', ''))[:50]}",
            f"Score={round(chosen['_score']):,}  Price=€{chosen['_p']:.2f}  Cap=€{cap_used:.0f}",
        ))
 
    # ═══════════════════════════════════════════════════════════
    # PASS 2 — Overflow at slot 11+, with spec variety enforced.
    # ═══════════════════════════════════════════════════════════
    overflow_slot_id = 11
    cursor = 0
    safety_counter = 0
    MAX_OVERFLOW_ATTEMPTS = len(SLOTS) * 3
    ...
    while len(all_recs_by_slot) < TARGET_FILLED_SLOTS and safety_counter < MAX_OVERFLOW_ATTEMPTS:
        ...
        for attempt in range(len(SLOTS)):
            ...
            slot_notes[overflow_slot_id] = (
                [f"🔁 Extra from slot {src_slot_num} ({src_role})"]
                + notes
            )
            lock_flag = "🔒" if locked else ""
            diag.append((
                f"Slot {overflow_slot_id} 🔁{lock_flag}",
                f"Extra from slot {src_slot_num} ({src_role})  →  {str(chosen.get('Title', ''))[:40]}",
                f"Score={round(chosen['_score']):,}  Price=€{chosen['_p']:.2f}  Sig={_signature(chosen, hierarchy)}",
            ))
    ...
    if all_recs_by_slot:
        recs_df = pd.DataFrame(
            [all_recs_by_slot[n] for n in sorted(all_recs_by_slot.keys())]
        )
    else:
        recs_df = pd.DataFrame()
...
# ═══════════════════════════════════════════════════════════════
# 6.  DIAGNOSTIC PRETTY-PRINT HELPER
# ═══════════════════════════════════════════════════════════════
...
def print_wearables_diag(diag: list, slot_notes: dict, score_breakdown: dict) -> None:
    ...
    for slot_num, notes in sorted(slot_notes.items()):
        print(f"  Slot {slot_num:>2}:  {' | '.join(notes)}")
 
    print("\n" + "─" * 72)
    print("  SCORE BREAKDOWN")
    print("─" * 72)
    for slot_num, bd in sorted(score_breakdown.items()):
        lock = "🔒" if bd.get("locked") else "  "
        print(
            f"  Slot {slot_num:>2} {lock} [{bd['slot_role']:<32}]  "
            f"Score={bd['final_score']:>10,}  "
            f"€{bd['price']:<7} cap€{bd['cap']:<6}  "
            f"Fb={bd['fallback']}  Sig={bd.get('signature', '')}"
        )
    print("═" * 72 + "\n")
...
# ═══════════════════════════════════════════════════════════════
# Per user spec (non-negotiable):
#   • Accessories (Εξαρτήματα για σκούπες): STRICT match —
#       Για μάρκες ηλεκτρικής σκούπας == trigger.Κατασκευαστής
#       AND Συμβατό μοντέλο == trigger.Μοντέλο
#     Universal types (αρωματικά / πέρλες) allowed as fallback because
#     they genuinely work on any vacuum.
#   • Companion devices (Stick/Σκουπάκια/Ηλεκτρικές/Ατμοκαθαριστές):
#       sales-based, with brand boost (+50K same Κατασκευαστής as trigger)
#       and price-tier proximity (+40K same tier, +15K ±1 tier).
...
def _rv_build_accessory_pool(c_acc, trigger_brand, trigger_model, notes):
    """Score the accessory pool. STRICT eligibility — per user spec:

        first spec of the accessories to match exactly the spec of the
        vacuum template:
          • Για μάρκες ηλεκτρικής σκούπας == Κατασκευαστής of the vacuum
          • AND Συμβατό μοντέλο == Model of the vacuum

    Anything that fails either condition is EXCLUDED — no aftermarket
    stick-vacuum accessories falsely labeled as Dyson-compatible robot
    accessories, no cross-brand specialty items.

    Universal types (αρωματικά sticks / πέρλες) survive as a soft fallback
    because they genuinely work on any vacuum brand/model. When the strict
    pool is empty and universal is too small, the round-robin loop will
    pull extra companion-device items into the slots instead.

    Returns sorted DataFrame ready for round-robin consumption."""
    if c_acc.empty:
        return c_acc
    ...
    if trigger_model and trigger_model.lower() not in ('n/a', 'nan', '', '0'):
        ...
        # ROHNSON quirk: "RΜ-05" uses Greek Μ; data may use Latin M
        tm_alt = tm_norm.replace('Μ', 'M')
        for mcol in ['Συμβατό μοντέλο', 'Συμβατό μοντέλο.1']:
            if mcol in pool.columns:
                ...
                model_mask |= col_vals.str.contains(re.escape(tm_norm), regex=True, na=False) | \
    ...
    # ── UNIVERSAL fallback (always allowed — αρωματικά work on any vacuum)
    ...
    if 'Τύπος συσκευής' in pool.columns:
        ...
        notes.append(f"  Universal types (αρωματικά/πέρλες): {universal_mask.sum()}")

    eligible = strict_mask | universal_mask
    pool = pool[eligible].copy()
    notes.append(f"  → After STRICT filter (strict_AND OR universal): {len(pool)} accessories")

    if pool.empty:
        notes.append(f"  ❌ Zero eligible accessories — loop will redirect slots to companion pools")
        return pool
    ...
    # ── Sales tiebreaker (small weight)
    pool['Final_Score'] += pool['Sales_Tiebreaker'].fillna(0) * RV_S_SALES_FACTOR

    # ── Tag the source mask back onto the surviving rows so we can score
    is_strict     = strict_mask.reindex(pool.index, fill_value=False)
    is_universal  = universal_mask.reindex(pool.index, fill_value=False)

    # Strict matches always score above universal fallbacks
    pool.loc[is_strict,    'Final_Score'] += RV_S_MODEL_EXACT
    pool.loc[is_universal, 'Final_Score'] += RV_S_UNIVERSAL_TYPE
...
# ═══════════════════════════════════════════════════════════════
# 🟢 TRADITIONAL VACUUMS ENGINE — Ηλεκτρικές Σκούπες (Μικρές Συσκευές)
# ═══════════════════════════════════════════════════════════════
# Same non-negotiable rules as Robot Vacuums:
#   • STRICT accessory match — Για μάρκες ηλεκτρικής σκούπας AND Συμβατό μοντέλο
#     both must match the trigger. Used for Σακούλες and Εξαρτήματα slots.
#   • UNIVERSAL αρωματικά / πέρλες in their own dedicated slot.
#   • Companions (Σκουπάκι/Stick/Robot/Ατμοκαθαριστής): sales + brand boost
#     + price-tier proximity. Σκούπες Stick and Σκούπες ρομπότ capped at 1
#     slot each.
#   • PET CARE gated on Κατάλληλη για κατοικίδια == Ναι.
...
def _tv_build_strict_accessory_pool(base, trigger_brand, trigger_model,
    ...
    if exclude_universal_types and 'Τύπος συσκευής' in pool.columns:
        ...
        notes.append(f"  After dropping universal αρωματικά: {strict_mask.sum()}")

    pool = pool[strict_mask].copy()
    notes.append(f"  → Final strict pool: {len(pool)}")
    ...
    if 'AVAILABILITY' in pool.columns:
        pool.loc[pool['AVAILABILITY'] == 'Άμεσα Διαθέσιμο', 'Final_Score'] += RV_S_AVAILABILITY
    pool['Final_Score'] += pool['Sales_Tiebreaker'].fillna(0) * RV_S_SALES_FACTOR
    pool['Final_Score'] += RV_S_MODEL_EXACT  # all rows here are strict matches

    return pool.sort_values('Final_Score', ascending=False)


def _tv_build_universal_aroma_pool(base, notes):
    """Pull only universal αρωματικά (sticks + πέρλες) from Εξαρτήματα.
    These work on any vacuum brand, no compatibility check needed."""
    if base.empty:
        return base
    pool = base.copy()
    if 'Τύπος συσκευής' not in pool.columns:
        notes.append("  ❌ No Τύπος συσκευής column — cannot identify αρωματικά")
        return pd.DataFrame()

    types_lower = pool['Τύπος συσκευής'].fillna('').astype(str).str.lower().str.strip()
    mask = types_lower.isin(ROBOT_VAC_UNIVERSAL_TYPES)
    pool = pool[mask].copy()
    notes.append(f"  Universal αρωματικά/πέρλες available: {len(pool)}")
    ...
    if 'AVAILABILITY' in pool.columns:
        pool.loc[pool['AVAILABILITY'] == 'Άμεσα Διαθέσιμο', 'Final_Score'] += RV_S_AVAILABILITY
    pool['Final_Score'] += pool['Sales_Tiebreaker'].fillna(0) * RV_S_SALES_FACTOR
    pool['Final_Score'] += RV_S_UNIVERSAL_TYPE
    return pool.sort_values('Final_Score', ascending=False)


def run_traditional_vacuums_engine(trigger, df_floor, df_history):
    """Build up to 10 cross-sell slots for a traditional vacuum trigger.

    Priority order follows the user spec:
      1. Σακούλες (strict brand+model)        2. Αρωματικά (universal)
      3. Σκουπάκι (companion)                  4. Σκούπα Stick (companion, cap=1)
      5. Εξαρτήματα (strict brand+model)       6. Σκούπα Ρομπότ (companion, cap=1)
      7. Ατμοκαθαριστής (companion)            8. PET CARE (sales, pet-gated)

    Round-robin loop: round 1 honors max_in_round_1 (Σακούλες & Εξαρτήματα
    each up to 2); subsequent rounds take 1 per pool until target hit."""
    diag = []
    slot_notes = {}
    all_recs = []
    ...
    # ── Drop the trigger + other traditional vacuums (these are competitors)
    c = df_floor[df_floor['Material'] != tm].copy()
    trigger_hiers = {h.upper().strip() for h in TRAD_VAC_TRIGGER_HIERARCHIES}
    b4 = len(c)
    c = c[~c['Hierarchy'].fillna('').astype(str).str.upper().str.strip().isin(trigger_hiers)]
    diag.append(("1. Excl traditional vacuums", len(c), f"Removed {b4 - len(c)} competitors"))
    ...
    for rank, role_label, hiers, logic_key, max_r1, max_total in TRAD_VAC_PRIORITY:
        notes = [f"=== Priority {rank}: {role_label} ({logic_key}) "
                 f"| max_round_1={max_r1} | max_total={max_total if max_total else '∞'} ==="]
        ...
        if logic_key == 'STRICT_ACCESSORY':
            ...
            # Σακούλες doesn't have universal types so the flag is harmless either way.
    ...
    while slot_num < TRAD_VAC_SLOT_TARGET:
        progress = False
        round_idx += 1
        for rank, (role_label, scored, logic_key, max_r1, max_total, notes) in pools.items():
            if slot_num >= TRAD_VAC_SLOT_TARGET:
                break
            if scored is None or scored.empty:
                continue
            if max_total is not None and pool_taken[rank] >= max_total:
            ...
            taken_this_pass = 0
            while taken_this_pass < take_n and cursor < len(scored) \
                  and slot_num < TRAD_VAC_SLOT_TARGET:
                row = scored.iloc[cursor]
                cursor += 1
                if row['Material'] in used_materials:
                    continue
                ...
                rc['Assigned_Slot'] = slot_num
                rc['Slot_Role'] = role_label
                rc['Marketing_Copy'] = TRAD_VAC_MARKETING_COPY.get(role_label, "Ιδανική επιλογή!")
                rc['Item_Rank'] = round_idx
                all_recs.append(rc)
                used_materials.add(row['Material'])
                taken_this_pass += 1
    ...
    # ── Append pool diagnostics under slot 0
    pool_diag_notes = []
    for rank, (role_label, scored, logic_key, max_r1, max_total, notes) in pools.items():
        pool_diag_notes.extend(notes)
        cap_note = f" (capped at {max_total})" if max_total is not None else ""
...
#   STR_HAIRSTYLING       — Hair Dryers / Curlers / Multistylers
#   STR_PCARE_ACCESSORY   — Personal Care Accessories
#   STR_WOMENS_CARE       — Epilators (and future women's-care hierarchies)
#   STR_WELLNESS          — Toothbrushes / Scales / Massage
#
# All four share the same scoring spine: AVAILABILITY → SALES → BRAND_MATCH →
# PRICE_TIER → COLOR_GROUP. They differ in their weights (STR_HAIRSTYLING
# weights color higher because Hair Dryers/Curlers/Multistylers are
# fashion-coordinated; STR_WELLNESS de-emphasises color because a body scale
# in a different color doesn't feel mismatched). The multistyler-upsell
# nudge fires only inside STR_HAIRSTYLING when the trigger itself is a
# multi-function straightener.

def _str_apply_base_score(pool, trigger_brand, trigger_tier, trigger_colors, notes,
                          color_exact_weight=STR_S_COLOR_EXACT,
                          color_partial_weight=STR_S_COLOR_PARTIAL):
    """Apply the shared availability + sales + brand + price-tier + color scoring.
    Returns the pool with a Final_Score column added. Mutates `notes`.
    """
    if pool.empty:
        return pool
    ...
    if 'AVAILABILITY' in pool.columns:
        ...
        pool.loc[avail_mask, 'Final_Score'] += STR_S_AVAILABILITY
        if avail_mask.any():
            notes.append(f"  ✓ Availability: {avail_mask.sum()} in stock (+{STR_S_AVAILABILITY:,})")

    # ── Base sales score (tiebreaker spine)
    pool['Final_Score'] += pool['Sales_Tiebreaker'].fillna(0) * STR_S_SALES_FACTOR

    # ── Brand-ecosystem boost (the strongest cross-sell signal)
    if trigger_brand and 'Κατασκευαστής' in pool.columns:
        same_brand = pool['Κατασκευαστής'].fillna('').astype(str).str.upper().str.strip() == trigger_brand
        pool.loc[same_brand, 'Final_Score'] += STR_S_BRAND_MATCH
        if same_brand.any():
            notes.append(f"  ✓ Brand ecosystem ({trigger_brand}): {same_brand.sum()} (+{STR_S_BRAND_MATCH:,})")

    # ── Price-tier proximity (same tier → boost, ≥2 away → penalty)
    if 'LIST PRICE' in pool.columns:
        prices = pool['LIST PRICE'].apply(parse_euro_price)
        ...
        near_tier = diffs == 1
        far_tier = diffs >= 2
        pool.loc[same_tier, 'Final_Score'] += STR_S_PRICE_SAME_TIER
        pool.loc[near_tier, 'Final_Score'] += STR_S_PRICE_ONE_OFF
        pool.loc[far_tier, 'Final_Score'] += STR_S_PRICE_TWO_OFF
        if same_tier.any() or near_tier.any() or far_tier.any():
            notes.append(f"  ✓ Price-tier match (trigger tier {trigger_tier}): "
                         f"same={same_tier.sum()} (+{STR_S_PRICE_SAME_TIER:,}), "
                         f"near={near_tier.sum()} (+{STR_S_PRICE_ONE_OFF:,}), "
                         f"far={far_tier.sum()} ({STR_S_PRICE_TWO_OFF:+,})")

    # ── Color-group match (aesthetic coordination)
    if trigger_colors and 'Χρώμα' in pool.columns and color_exact_weight > 0:
        pool_colors = pool['Χρώμα'].apply(_str_color_group)
        # Exact = identical group set (e.g. {'PINK'} == {'PINK'})
        exact_mask = pool_colors.apply(lambda g: g == trigger_colors and len(g) > 0)
        # Partial = any token overlap but not full equality
        partial_mask = pool_colors.apply(
            lambda g: bool(g & trigger_colors) and g != trigger_colors
        )
...
def _str_build_hairstyling_pool(c_pool, trigger_brand, trigger_tier,
                                 trigger_colors, is_multistyler_trigger,
                                 role_label, notes):
    """Score a hair-styling companion pool (Hair Dryers / Curlers / Multistylers).
    Color matters MOST here — these are coordinated styling tools women buy
    as a matching set. We also fire the multistyler-upsell nudge when the
    trigger is a multi-function straightener AND we're scoring the
    Πολυσυσκευή Styling slot.
    """
    if c_pool.empty:
        return c_pool

    pool = _str_apply_base_score(
        c_pool, trigger_brand, trigger_tier, trigger_colors, notes,
        color_exact_weight=STR_S_COLOR_EXACT,
        color_partial_weight=STR_S_COLOR_PARTIAL,
    )

    # ── Multistyler-upsell nudge: only fires when scoring the MULTISTYLERS slot
    # AND the trigger is already a multi-function straightener. The user has
    # signalled they want versatility — push the premium multistylers higher
    # so they see the pro-grade upgrade (Dyson Airwrap, BaByliss Big Hair, etc).
    if is_multistyler_trigger and role_label == 'Πολυσυσκευή Styling' and 'LIST PRICE' in pool.columns:
        prices = pool['LIST PRICE'].apply(parse_euro_price)
        premium_mask = prices >= 200  # Multistylers €200+ are the pro tier
        pool.loc[premium_mask, 'Final_Score'] += STR_S_MULTISTYLER_UPSELL
        if premium_mask.any():
            notes.append(f"  ✓ Multistyler upsell (trigger is multi-function): "
                         f"{premium_mask.sum()} premium multistylers (+{STR_S_MULTISTYLER_UPSELL:,})")

    return pool.sort_values('Final_Score', ascending=False)


def _str_build_accessory_pool(c_pool, trigger_brand, trigger_tier,
                               trigger_colors, notes):
    """Score the Personal Care ACCESSORIES pool.
    Color is irrelevant here (most accessories are utilitarian Inox/Πράσινο/
    no-color) so we zero out the color weight. Brand match still matters
    (DYSON brushes for DYSON straightener owners, PHILIPS brushes for PHILIPS).
    """
    if c_pool.empty:
        return c_pool
...
def _str_build_womens_care_pool(c_pool, trigger_brand, trigger_tier,
                                  trigger_colors, notes):
    """Score the women's-care companions (Epilators today; future hierarchies
    can slot in here without changes).
    Color matters less than hair-styling — epilators are bathroom-shelf items,
    not part of the styling-set aesthetic — so we use the partial weight only.
    """
    if c_pool.empty:
        return c_pool
    ...
    pool = _str_apply_base_score(
        c_pool, trigger_brand, trigger_tier, trigger_colors, notes,
        color_exact_weight=STR_S_COLOR_PARTIAL,  # downgrade exact to partial
        color_partial_weight=STR_S_COLOR_PARTIAL,
    )
    return pool.sort_values('Final_Score', ascending=False)


def _str_build_wellness_pool(c_pool, trigger_brand, trigger_tier,
                              trigger_colors, notes):
    """Score the wellness companions (Toothbrushes / Scales / Massage).
    These hierarchies are dominated by DIFFERENT brands (ORAL-B, BEURER,
    THERABODY) — straightener brand-match rarely fires here. Falls back to
    price-tier + sales. Color is muted: a body scale in a different color
    from your straightener doesn't read as a mismatch.
    """
    if c_pool.empty:
        return c_pool
...
# ═══════════════════════════════════════════════════════════════
# 🟢 STRAIGHTENERS ENGINE — Ισιωτικά Μαλλιών (Personal Care)
# ═══════════════════════════════════════════════════════════════
# Mirror of run_air_fryer_engine: pre-build & score every pool from
...
def run_straighteners_engine(trigger, df_sda, df_history):
    ...
    is_multi = _str_is_multistyler_trigger(trigger)

    diag.append(("0. Trigger", f"{tb} €{tprice:.0f}",
                 f"Model={tmodel} | Tier={ttier} | Color={tcolor_raw} → {sorted(tcolors)} | Multistyler={is_multi}"))

    if df_sda is None or df_sda.empty:
        diag.append(("ERROR", 0, "SDA sheet is empty — engine cannot run"))
        return pd.DataFrame(), diag, slot_notes, pd.DataFrame()

    # ── Drop the trigger itself + any other straighteners (competitors)
    c_sda = df_sda[df_sda['Material'] != tm].copy()
    trigger_hiers = {h.upper().strip() for h in STRAIGHTENER_TRIGGER_HIERARCHIES}
    b4 = len(c_sda)
    c_sda = c_sda[~c_sda['Hierarchy'].fillna('').astype(str).str.upper().str.strip().isin(trigger_hiers)]
    diag.append(("1. Excl straighteners", len(c_sda), f"Removed {b4 - len(c_sda)} competitor straighteners"))
    ...
    for rank, role_label, hiers, logic_key, max_r1, max_total in STRAIGHTENER_PRIORITY:
        ...
        if logic_key == 'STR_HAIRSTYLING':
            scored = _str_build_hairstyling_pool(
                base_pool, tb, ttier, tcolors, is_multi, role_label, notes
            )
        elif logic_key == 'STR_PCARE_ACCESSORY':
    ...
    while slot_num < STRAIGHTENER_SLOT_TARGET:
        progress = False
        round_idx += 1
        for rank, (role_label, scored, logic_key, max_r1, max_total, notes) in pools.items():
            if slot_num >= STRAIGHTENER_SLOT_TARGET:
                break
            if scored is None or scored.empty:
                continue
            ...
            taken_this_pass = 0
            while taken_this_pass < take_n and cursor < len(scored) \
                  and slot_num < STRAIGHTENER_SLOT_TARGET:
                row = scored.iloc[cursor]
                cursor += 1
                if row['Material'] in used_materials:
                    continue
                ...
                rc['Assigned_Slot'] = slot_num
                rc['Slot_Role'] = role_label
                rc['Marketing_Copy'] = STRAIGHTENER_MARKETING_COPY.get(role_label, "Ιδανική επιλογή!")
                rc['Item_Rank'] = round_idx
                all_recs.append(rc)
                used_materials.add(row['Material'])
                taken_this_pass += 1
    ...
    diag.append(("TOTAL", len(all_recs),
                 f"Filled {slot_num}/{STRAIGHTENER_SLOT_TARGET} slots in {round_idx} rounds"))
...
with st.expander("⚙️ System Diagnostics"):
    ...
    if active_cluster == "Kids Books":
        ...
        # The unstructured Δυνατότητες / Ειδικά χαρακτηριστικά columns are
        # shown if non-empty but the engine doesn't read them.
