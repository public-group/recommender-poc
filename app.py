...
# ───── Product selector + trigger setup based on active_cluster ─────
    ...
    if active_cluster == "Smartphones":
        ...
        if projs.empty:
            ...
            # Φιλτράρισμα για την κατηγορία ΠΙΚΑΠ
            lvl2 = df_products['Level 2'].fillna('').astype(str).str.strip().str.upper()
            turntables = df_products[lvl2 == 'ΠΙΚΑΠ']
    
            # ─────────────────────────────────────────────────────────────
            # 🧪 TEST LIST: Restrict the dropdown to specific SKUs
            # ─────────────────────────────────────────────────────────────
            tt_test_skus = {
                "1956497", "2106285", "1821326", "1402320", "1873884"
            }
            ...
            if turntables.empty:
                st.sidebar.warning("Δεν βρέθηκαν test Πικάπ.")
            else:
